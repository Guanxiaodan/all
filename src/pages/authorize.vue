...
<template>
  <div class="flex-c center container-all">
    ...
    <div v-if="!showMag.客户名">
      ...
      <div>
        <Button type="primary" class="to-top-large" :disabled="isSending" @click="sendreq">{{isSending? '正在解析':'解析请求码'}}</Button>
    ...
    <div v-if="showMag.客户名">
      <h2>请求码：</h2>
      <div style="width: 30rem; height: 10rem;overflow: scroll;border: 1px solid #ccc;word-wrap:break-word;text-align: left">
        {{requestCode}}
      </div>
    </div>
    <!--填写生成授权码的按相关信息-->
    <div v-if="showMag.客户名 && !authorizeCode" class="to-top-large">
      <div class="flex-r">
        <h2>请选择截止日期：</h2>
        <DatePicker type="date" @on-change="changeDate" :options="options4" placeholder="请选择截止日期" style="width: 200px"></DatePicker>
      </div>
      ...
      <div>
        <Button type="primary" class="to-top-large" :disabled="isSending" @click="getAuthorize">{{isSending? '正在获取...':'获取授权码'}}</Button>
        <Button type="primary" class="to-top-large to-left-large" @click="back">返 回</Button>
      </div>
    </div>
    <!--显示授权码-->
